...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.manual_button = QPushButton("Previous frame")
        self.manual_button.clicked.connect(self.previous_frame)
        self.layout.addWidget(self.manual_button, 3, 0)

        self.manual_button = QPushButton("Next frame")
        self.manual_button.clicked.connect(self.next_frame)
        self.layout.addWidget(self.manual_button, 3, 1)

        self.auto_button = QPushButton("Auto Play")
        self.auto_button.setCheckable(True)
        self.auto_button.clicked.connect(self.toggle_auto_play)
        self.layout.addWidget(self.auto_button, 3, 2)
    ...
    def on_canvas_click(self, event):
        if event.inaxes == self.ax:
            ...
            if 0 <= neuron_idx < len(self.signals):
                ...
                self.update_images(self.current_frame)
            f_click = int(event.xdata)
            if  0 <= f_click < len(self.images):
                self.current_frame = f_click
                self.slider.setValue(self.current_frame)
